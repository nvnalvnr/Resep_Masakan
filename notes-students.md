...
**Cara benerin (langkah-langkah):**

1. Buka `database/seeders/RecipeSeeder.php`
2. Paling atas file, tambah baris `use` buat helper slug:
    ```php
    use Illuminate\Support\Str;
    ```
3. Di tiap `Recipe::create([...])` — ada 7 blok, satu per resep — tambah
   satu baris `'slug' => Str::slug('Judul Resepnya'),` persis di bawah
   baris `'title'`. Contoh buat resep pertama:
    ```php
    Recipe::create([
        'user_id' => $admin->id,
        'title' => 'Nasi Goreng Spesial',
        'slug' => Str::slug('Nasi Goreng Spesial'),   // ← baris baru
        'ingredients' => "...",
        'steps' => "...",
    ]);
    ```
4. Ulangi langkah 3 buat 6 resep sisanya — judul di `Str::slug(...)`
   harus sama persis dengan judul di `'title'` supaya nggak typo
5. Simpan file, lalu tes ulang:
    ```
    php artisan migrate:fresh --seed
    ```
6. Kalau semua 7 resep masuk tanpa error `slug` — berarti sudah benar

**Kenapa harus manual tiap resep, bukan taruh di satu tempat?** Karena
`Str::slug()` butuh tahu judul resep yang mana buat diubah jadi slug —
tiap resep judulnya beda, jadi tiap `Recipe::create()` butuh baris
slug-nya sendiri-sendiri.
...
**Satu pelajaran utama:** kamu mengecek kode _jalan_, bukan fitur _berfungsi_.
...
**Catatan tambahan:** kamu memperbaiki semua yang _terlihat di layar_ dan
